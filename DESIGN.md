...
description: An inspired interpretation of Vercel's design language — a developer-platform brand whose surface is a purple-on-white system with deep purple #501cbe as the primary accent on a clean white canvas, broken at hero scale by a multi-color mesh gradient (cyan / blue / magenta / amber) that acts as the entire decorative system, paired with a custom geometric sans for headlines and a monospaced caption face for technical labels.
...
Surfaces use a four-step ladder: `{colors.canvas}` (pure white for cards), `{colors.canvas-soft}` 98% (the page body), `{colors.canvas-soft-2}` 95% (occasional inset region), `{colors.primary}` (the deep purple `#501cbe` used as the polarity-flipped band when a section needs the dark mode treatment). Shadows are exceptionally subtle — every elevated card carries a stacked shadow built from `0px 1px 1px #00000005` + `0px 2px 2px #0000000a` + an inset border. Cards never float on heavy drop-shadow; they sit on the page held by hairline + soft glow.

**Key Characteristics:**
- A single purple primary CTA `{colors.primary}` carries every conversion target, paired with white-on-white `button-secondary` for the secondary action. The brand uses 100 px pill shape for marketing CTAs and a tight 6 px square shape for in-app nav buttons.
...
### Brand & Accent
- **Purple Primary** (`{colors.primary}` — `#501cbe`): The single primary CTA color. Deep purple that carries every Sign Up pill, every footer CTA, the dark-band polarity-flip. Used as the brand accent throughout the page.
...
- **Link Purple** (`{colors.link}` — `#501cbe`): The brand's primary link color and the success semantic.
...
### Semantic
- **Success / Link** (`{colors.success}` — `#501cbe`): The brand's success indicator doubles as the primary link color. Visible underline-on-hover for inline body links.
- **Link Deep** (`{colors.link-deep}` — `#3d1496`): The pressed / visited tone for inline links.
- **Link Bg Soft** (`{colors.link-bg-soft}` — `#e8dff7`): Soft pastel purple fill for "what's new" pill banners and informational badges.
...
- **Polarity-flipped purple band as section-depth**: switching the surface from `{colors.canvas-soft}` to `{colors.primary}` (the deep purple `#501cbe`) is the brand's chief depth cue between bands.
...
**`button-primary`** — the canonical 100-px-radius purple pill, marketing scale.
...
**`button-secondary`** — the white pill paired with the purple primary inside marketing bands.
...
**`nav-cta-signup`** — the small purple "Sign Up" button in the nav row.
...
**`code-editor-mockup`** — the purple code-preview surface inside marketing bands.
...
- Background `{colors.primary}`, text `{colors.on-primary}`, same shape + padding as `pricing-card`. CTA inverts to `button-secondary-sm` (white pill on purple card).
...
**`showcase-band-dark`** — the polarity-flipped purple band ("A compute model for all workloads").
- Background `{colors.primary}`, text `{colors.on-primary}`, padding `{spacing.5xl} {spacing.lg}`. Section headline in `{typography.display-lg}` (white on purple). Often contains a `code-editor-mockup` flush with the band.
...
- Text `{colors.link}` (`#501cbe`), body in `{typography.body-md}`, underlined.
...
### Do
- Reserve `{colors.primary}` (`#501cbe`) for primary CTAs across the page. Purple `#501cbe` IS the conversion target.
